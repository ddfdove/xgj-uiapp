	...
	<view class="write_list_page">
		<div class="write_list_nav">
			<div class="write_list_nav_view" v-if="menuButtonInfo"
				...
				<text
					style="margin-left: 80px;color: #1A1A1A;font-size: 18px; font-weight: 500;font-family: Source Han Sans CN;text-align: center;">加油</text>
			</div>
		</div>
		<div class="write_list_content">
			<div>
				<div class="write_list_content_top">
					<u-search placeholder="输入单号或加油升数或公里数查询" prefixIcon="search" placeholderStyle=";color:#1A1A1A"
			...
			<div>
				<div class="write_list_content_item" v-for="(item,index) in list" :key="index"
					@click="goToPage(`/pages/vehicle/refuel/info?id=${item. drivingId}`)">
					...
					<div class="write_list_content_item_top" style="flex:1;display: flex;margin-top: 20px;">
						<span style="margin-left: 10px;color: #1A1A1A;font-size: 18px; font-weight: 500;flex:1.8">单号：{{item.orderNumber}}</span>
						
					</div>
					<span style="margin-left: 220px;margin-top:10px;flex:1">公里数：{{item.kilometreNumber}}</span>
					<div style="flex:1;display: flex;">
						<span style="flex:1.6;margin-left: 10px;">加油升数：{{item.oilNumber}}</span>
