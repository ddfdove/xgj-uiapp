	...
	<view class="write_list_page">
		<div class="write_list_nav">
			<div class="write_list_nav_view" v-if="menuButtonInfo"
				...
				<text
					style="margin-left: 100px;color: #1A1A1A;font-size: 18px; font-weight: 500;font-family: Source Han Sans CN;">修理</text>
			</div>
		</div>
		<div class="write_list_content">
			<div>
				<div class="write_list_content_top">
					<u-search placeholder="请输入单号查询" prefixIcon="search" placeholderStyle=";color:#1A1A1A"
			...
			<div>
				<div class="write_list_content_item" v-for="(item,index) in list" :key="index"
					...
					<span style="margin-left: 220px;margin-top:10px;flex:1">公里数：{{item.kilometreNumber?item.kilometreNumber:'无'}}</span>
...
<script>
	...
	export default {
		...
		methods: {
			...
			asyncGetList() {
				repairList(this.listParam).then(res => {
					console.log(res);
					if (res.code == 0) {
						console.log(res)
