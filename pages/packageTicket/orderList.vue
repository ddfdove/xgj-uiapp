<template>
	<view class="write_list_page">
		<div class="write_list_nav">
			<div class="write_list_nav_view" v-if="menuButtonInfo" :style="{height:menuButtonInfo.height+'px',top:menuButtonInfo.top+'px'}">
				...
				<div :style="{height:menuButtonInfo.height+'px'}" v-if="type==2">
					<u--input placeholder="搜套票" prefixIcon="search" placeholderStyle="background-color:#F0F5FF;" prefixIconStyle="font-size: 26px;color: #6C7B92" shape="circle"></u--input>
				</div>
			</div>
		</div>
		<div class="write_list_content">
			...
			<!-- -->
			<div class="write_list_content_item" v-for="(item,index) in list" :key="index" @click="goToPage(`/pages/packageTicket/orderInfo?ticketNumber=${item.packageTicketSnowRecord.id}&type=list`)" >
				<div class="write_list_content_item_top" >
					<span>核销码：<span v-text="item.snowTicketInfo&&item.snowTicketInfo.code"></span></span>
					<text >购票详情</text>
				</div>
				<div class="write_list_content_item_content">
					<div class="content_left">
						<image style="width:100%;height:100%;" :src="(item.snowTicketInfo&&item.snowTicketInfo.extraProperties.PictureUrl)?item.snowTicketInfo.extraProperties.PictureUrl:'https://vankebj.oss-cn-beijing.aliyuncs.com/tenants/ae9c54cc-94e0-5552-ef06-3a044ab7a302public/133096916294989653.png'" mode=""></image>
					</div>
					<div class="content_right">
						<p v-text="item.snowTicketInfo&&item.snowTicketInfo.ticketName">2022年温泉镇第五届红楼迷马山…</p>
						<div>
							<p>￥<span v-text="item.snowOrderInfo&&item.snowOrderInfo.orderTotal">358.00</span></p>
							<text v-text="item.snowOrderInfo&&item.snowOrderInfo.ticketCount">× 1</text>
						</div>
						<span>核销时间 <span v-if="item.packageTicketSnowRecord">{{item.packageTicketSnowRecord.verificationTime}}</span></span>
					</div>
				</div>
			</div>
		</div>
...
<script>
	...
	import { mineSnowOrderList} from "@/api/index.js" 
	...
	export default {
		...
		methods:{
			...
			asyncGetList(){
				mineSnowOrderList(this.listParam).then(res=>{
					if(res.code==200){
						let data = res.data;
